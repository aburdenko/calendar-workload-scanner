...
if [ -z "$APP_SCRIPT_IDS" ]; then
  ...
  if [ -n "$NEW_SCRIPT_ID" ]; then
    ...
    cat <<EOF > apps-script/Env.js
// Auto-generated by deploy_apps_script.sh from .env
const ENV = {


  WORKLOAD_LINK_PREFIX: "${WORKLOAD_LINK_PREFIX:-https://vector.lightning.force.com/lightning/r/Workload__c/}"
};
EOF

    echo "Pushing code to the new script..."
    clasp push --force
    echo "Deployment complete! Please go to https://script.google.com/d/$NEW_SCRIPT_ID/edit to run setupTrigger() manually once."
  else
